...
if model is None:
    st.error("Model files not found. Make sure `deepfake_cnn.pt` and `config.json` are in the repo.")
    st.stop()
...
if uploaded:
    ...
    librosa.display.specshow(lm, x_axis="time", y_axis="mel",
        sr=SR, hop_length=config["HOP_LENGTH"], ax=ax, cmap="magma")
    ax.set_title("Genuine" if pred==0 else "Deepfake")
